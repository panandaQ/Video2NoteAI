...
chat_api_key="${CHAT_API_KEY:-${DEEPSEEK_API_KEY:-${SILICONFLOW_API_KEY:-}}}"
embedding_api_key="${EMBEDDING_API_KEY:-${SILICONFLOW_API_KEY:-}}"
asr_api_key="${ASR_API_KEY:-${SILICONFLOW_API_KEY:-}}"
for provider_key in chat_api_key embedding_api_key asr_api_key; do
  value="${!provider_key:-}"
  if [[ -z "$value" || "$value" == change-* ]]; then
    echo "Set a usable credential for $provider_key in .env" >&2
    exit 1
  fi
done
