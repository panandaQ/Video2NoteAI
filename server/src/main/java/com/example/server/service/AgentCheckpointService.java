...
public class AgentCheckpointService {
    ...
    /**
     * 读取 V2 内容上下文（只读 V2 键，不回退 V1——D-079）。
     * 自定义目标需要 V1 只读复用时由 {@link AiService} 显式调用 {@link #loadLegacyContext(Long)}。
     */
    public VideoContext loadContext(Long mediaId) {
        return checkpointRepository.read(mediaId, mediaCheckpointV2("context"),
                checkpointKey(mediaId), "context", VideoContext.class);
    }

    /** 严格 V1 读取（只读，供自定义目标兼容路径使用，D-077）。 */
    public VideoContext loadLegacyContext(Long mediaId) {
        return checkpointRepository.read(mediaId, mediaCheckpoint("context"),
                checkpointKey(mediaId), "context", VideoContext.class);
    }
    ...
    /**
     * 该媒体最近一次检查点写入时间，没有记录时返回 {@code null}。
     *
     * <p>供导入恢复扫描判断"持有活跃键的分析进程是否还活着"：一个进程被杀掉后，活跃键会一直留到 TTL 到期
     * （默认 6 小时），期间任何重投都会被"已有活跃任务"挡回。检查点是否仍在推进是唯一可靠的存活证据。
     * 本方法只读，不写任何检查点。
     */
    public LocalDateTime loadLatestProgressAt(Long mediaId) {
        return checkpointRepository.readLatestUpdatedAt(mediaId);
    }
    ...
    public List<VideoChunk> loadChunks(Long mediaId) {
        // 只读 V2 键（D-079）：D-069 的"索引就绪"边界只认 V2 快照，V1 快照不回退。
        return checkpointRepository.read(
                mediaId,
                mediaCheckpointV2("chunks"),
                checkpointKey(mediaId),
                "chunks",
                new TypeReference<List<VideoChunk>>() { });
    }

    /** 严格 V1 分块读取（只读，供兼容路径使用）。 */
    public List<VideoChunk> loadLegacyChunks(Long mediaId) {
        return checkpointRepository.read(
                mediaId,
    ...
    public void saveContext(Long mediaId, VideoContext context) {
        // V2 内容检查点必须保留时长/版本/章节（交付约束 2：不得把"有章节"静默降级成"无章节"）
        ...
        checkpointRepository.write(mediaId, mediaCheckpointV2("context"), mediaCheckpoint("stage"),
    ...
    /** V2 内容级检查点字段（计划 §6.2）：V1 键保留只读，不被 V2 自动复用。 */
    private String mediaCheckpointV2(String field) {
        return "media:" + field + ":v2";
    }
