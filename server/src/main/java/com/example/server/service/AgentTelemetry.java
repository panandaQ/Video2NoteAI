...
public class AgentTelemetry {
    ...
    public String start(Long taskId, String goal, AnalysisMode mode) {
        return register(UUID.randomUUID().toString(), taskId, goal, mode);
    }

    /**
     * 复用已有 traceId（问答链受理时生成的 {@code knowledge_turns.trace_id}）注册并绑定当前线程，
     * 使后续模型调用与阶段计时进入同一条 trace，而不是另起一个与 DB 记录割裂的 UUID。
     */
    public String register(String traceId, Long taskId, String goal) {
        return register(traceId, taskId, goal, AnalysisMode.GENERAL);
    }

    private String register(String traceId, Long taskId, String goal, AnalysisMode mode) {
    ...
    /** 记录当前绑定 trace 的一个非模型阶段耗时（模型阶段已由 {@link #modelCall} 自带 stage 记录）。 */
    public void stageCurrent(String stage, long startedNanos, boolean success) {
        String traceId = currentTrace.get();
        if (traceId != null) stage(traceId, stage, startedNanos, success);
    }
